...
export const BottomNav = ({ currentScreen, onNavigate }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto z-40">
      <div className="bg-[#00823B] text-white flex items-center justify-around py-2 px-3 shadow-2xl border-t border-emerald-600/60 backdrop-blur-md">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            ...
            >
              ...
              <span className="text-xs tracking-wide">{tab.name}</span>
            </button>
          );
        })}
      </div>
